...
| BGEN | `.bgen` plus optional same-prefix `.sample` | dense `kind="geno", dosage="dosage"` only | no | Real sample IDs are required from embedded identifiers or a `.sample` file. |
...
- `dosage="dosage"` currently supports dense VCF `FORMAT/DS` reads, dense
  ...
  unphased diploid dosage reads. Sparse dosage reads are not implemented.
...
- BGEN support is limited to dense `kind="geno", dosage="dosage"` reads.
  Hardcall conversion, sparse reads, haplotype reads, phased BGEN,
  multiallelic BGEN, variable ploidy, unsupported compression and layout values,
  and `.bgi` or region pushdown are not supported.
